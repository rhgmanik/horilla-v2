...
def currency_symbol_position(amount):
    ...
    if amount is None:
        ...
        if isinstance(amount, (int, float, Decimal)):
            try:
                ...
                decimals = 2 if value.as_tuple().exponent < 0 else 0
            except (InvalidOperation, ValueError):
                value = None
        elif isinstance(amount, str):
            raw = amount.strip()
            if raw:
                raw_clean = raw.replace("\xa0", " ").replace(" ", "")
                candidate = re.sub(r"[^\d,.\-+()]", "", raw_clean)
                is_negative = False
                ...
                decimal_digits = 0
                if decimal_sep and decimal_sep in candidate:
                    int_part, frac_part = candidate.rsplit(decimal_sep, 1)
                    if frac_part.isdigit():
                        decimal_digits = len(frac_part)
                        int_part = int_part.replace(",", "").replace(".", "")
                        normalized = f"{int_part}.{frac_part}" if decimal_digits else int_part
                    else:
                ...
                if is_negative and not normalized.startswith("-"):
                ...
                try:
                    ...
                    decimals = decimal_digits if decimal_digits else (2 if value.as_tuple().exponent < 0 else 0)
                except (InvalidOperation, ValueError):
                    value = None
        ...
        if value is None:
            ...
            if lang.startswith("id"):
                dpos = decimals or 0
                formatted_amount = format(value, f",.{dpos}f")
                formatted_amount = (
                    formatted_amount.replace(",", "X")
                    .replace(".", ",")
                    .replace("X", ".")
                )
            else:
                formatted_amount = formats.number_format(
                    value,
                    decimal_pos=decimals or 0,
                    use_l10n=use_l10n,
                    force_grouping=True,
                )
    ...
    if apps.is_installed("payroll"):
        PayrollSettings = get_horilla_model_class(app_label="payroll", model="payrollsettings")
        symbol = PayrollSettings.objects.first()
        if symbol:
            currency = symbol.currency_symbol or currency
            position = getattr(symbol, "position", position) or position

    if position == "postfix":
        return f"{formatted_amount} {currency}".strip()
    return f"{currency} {formatted_amount}".strip()
