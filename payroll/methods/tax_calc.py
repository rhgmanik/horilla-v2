...
_custom_tax_calculator = None


def _load_custom_tax_calculator():
    global _custom_tax_calculator
    if _custom_tax_calculator is not None:
        return _custom_tax_calculator

    hook = getattr(settings, "HORILLA_TAX_CALCULATOR", None)
    if not hook:
        _custom_tax_calculator = False
        return None

    if callable(hook):
        _custom_tax_calculator = hook
        return hook
...
def calculate_taxable_amount(**kwargs):
    ...
    custom_calc = _load_custom_tax_calculator()
    if custom_calc:
        try:
            custom_value = custom_calc(**kwargs, contract=contract, filing=filing)
