...
_payroll_period_context_resolver = None


def _load_payroll_period_context_resolver():
    global _payroll_period_context_resolver
    if _payroll_period_context_resolver is not None:
        return _payroll_period_context_resolver

    hook = getattr(settings, "HORILLA_PAYROLL_PERIOD_CONTEXT", None)
    if not hook:
        _payroll_period_context_resolver = False
        return None

    if callable(hook):
        _payroll_period_context_resolver = hook
        return hook
    ...
    if isinstance(hook, str):
        ...
        try:
            ...
            if callable(fn):
                _payroll_period_context_resolver = fn
                return fn
        except Exception:
            _payroll_period_context_resolver = False
            return None
...
def _get_payroll_period_context(*, employee, start_date, end_date):
    resolver = _load_payroll_period_context_resolver()
    if not resolver:
        return None
    try:
        data = resolver(employee=employee, start_date=start_date, end_date=end_date)
...
def monthly_computation(employee, wage, start_date, end_date, *args, **kwargs):
    ...
    total_working_days_on_period = sum(
        int(item.get("working_days_on_period") or 0) for item in month_data
    )
    ...
    paid_days = max(0, total_working_days_on_period - unpaid_leaves)

    context = _get_payroll_period_context(
        employee=employee, start_date=start_date, end_date=end_date
    )
    monthly_salary_mode = (
        (context or {}).get("monthly_salary_mode") or "prorate_by_calendar_month"
    )
    standard_period = (context or {}).get("standard_period") or {}

    if monthly_salary_mode == "full_period_as_one_month":
        standard_start = standard_period.get("start_date")
        standard_end = standard_period.get("end_date")
        if standard_start and standard_end:
            standard_working_days = get_working_days(standard_start, standard_end)[
                "total_working_days"
            ]
        else:
            standard_working_days = 0

        if standard_working_days:
            basic_pay = wage * (total_working_days_on_period / standard_working_days)
        else:
            basic_pay = 0

        if contract.calculate_daily_leave_amount:
            daily_salary = wage / standard_working_days if standard_working_days else 0
