...
_default_daterange_resolver = None


def _load_default_daterange_resolver():
    global _default_daterange_resolver
    if _default_daterange_resolver is not None:
        return _default_daterange_resolver

    hook = getattr(settings, "HORILLA_PAYSLIP_DEFAULT_DATERANGE", None)
    if not hook:
        _default_daterange_resolver = False
        return None

    if callable(hook):
        _default_daterange_resolver = hook
        return hook
    ...
    if isinstance(hook, str):
        ...
        try:
            ...
            if callable(fn):
                _default_daterange_resolver = fn
                return fn
        except Exception:
            _default_daterange_resolver = False
            return None
...
class GeneratePayslipForm(HorillaForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.initial["start_date"] = datetime.date.today().replace(day=1)
        self.initial["end_date"] = datetime.date.today()
        resolver = _load_default_daterange_resolver()
        if resolver:
            request = getattr(_thread_locals, "request", None)
            try:
                data = resolver(request=request)
            except TypeError:
                data = resolver()
            except Exception:
                data = None
            if isinstance(data, dict) and data.get("start_date") and data.get("end_date"):
                self.initial["start_date"] = data["start_date"]
                self.initial["end_date"] = data["end_date"]
        if not self.initial.get("group_name") and self.initial.get("start_date") and self.initial.get("end_date"):
            start_str = self.initial["start_date"].strftime("%Y-%m-%d")
            end_str = self.initial["end_date"].strftime("%Y-%m-%d")
            self.initial["group_name"] = f"Payroll {start_str} - {end_str}"
