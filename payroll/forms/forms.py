...
class ContractForm(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        elif self.data.get("employee_id"):
            try:
                employee = Employee.objects.filter(pk=self.data.get("employee_id")).first()
            except Exception:
                employee = None
        ...
        if employee:
            try:
                profile = employee.indonesia_profile
            except Exception:
                profile = None
            if profile:
                npwp = (profile.npwp or "").strip()
                nik = (profile.nik or "").strip()
                bpjs_kesehatan = (profile.bpjs_kesehatan or "").strip()
                bpjs_ketenagakerjaan = (profile.bpjs_ketenagakerjaan or "").strip()

        self.fields["indonesia_npwp"] = forms.CharField(
            label=_("NPWP"),
            required=False,
            initial=npwp,
            widget=forms.TextInput(attrs={"readonly": "readonly"}),
        )
        self.fields["indonesia_nik"] = forms.CharField(
            label=_("NIK (KTP/Paspor)"),
            required=False,
            initial=nik,
            widget=forms.TextInput(attrs={"readonly": "readonly"}),
        )
        self.fields["indonesia_bpjs_kesehatan"] = forms.CharField(
            label=_("BPJS Kesehatan"),
            required=False,
            initial=bpjs_kesehatan,
            widget=forms.TextInput(attrs={"readonly": "readonly"}),
        )
        self.fields["indonesia_bpjs_ketenagakerjaan"] = forms.CharField(
            label=_("BPJS Ketenagakerjaan"),
            required=False,
            initial=bpjs_ketenagakerjaan,
            widget=forms.TextInput(attrs={"readonly": "readonly"}),
        )
        ...
        for name, field in self.fields.items():
            ...
            if name == "employee_id":
                for extra in (
                    "indonesia_npwp",
                    "indonesia_nik",
                    "indonesia_bpjs_kesehatan",
                    "indonesia_bpjs_ketenagakerjaan",
                ):
                    if extra in self.fields and extra not in ordered:
                        ordered[extra] = self.fields[extra]
        self.fields = ordered
