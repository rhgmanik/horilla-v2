...
def payslip_detailed_export_data(request):
    ...
    allowance_titles_set = set()
    deduction_titles_set = set()
    for payslip in payslips:
        pay_head = payslip.pay_head_data or {}
        for allowance in pay_head.get("allowances") or []:
            title = (allowance or {}).get("title")
            title = str(title).strip() if title is not None else ""
            if title:
                allowance_titles_set.add(title)
        for deduction in all_deductions(pay_head):
            title = (deduction or {}).get("title")
            title = str(title).strip() if title is not None else ""
            if title:
                deduction_titles_set.add(title)

    allowance_titles = sorted(allowance_titles_set)
    deduction_titles = sorted(
        [t for t in deduction_titles_set if t.strip().lower() != "federal tax"]
    )

    selected_columns += [(title, title) for title in allowance_titles]
    selected_columns += [("total_allowances", "Total Allowances")]

    selected_columns += [(title, title) for title in deduction_titles]
    selected_columns += [
        ("federal_tax", "Federal Tax"),
        ("total_deductions", "Total Deductions"),
    ]

    allowance_totals = {title: 0 for title in allowance_titles}
    deduction_totals = {title: 0 for title in deduction_titles}
    ...
    return {
        ...
        "allowances": allowance_titles,
        "deductions": deduction_titles,
    }
...
def payslip_detailed_export(request):
    ...
    allowances_header = allowances + ["Total Allowances"]
