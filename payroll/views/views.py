...
def view_payslip_pdf(request, payslip_id):
    ...
    if Payslip.objects.filter(id=payslip_id).exists():
        payslip = Payslip.objects.select_related(
            ...
            "employee_id__employee_bank_details",
        ).get(id=payslip_id)
...
def contract_info_initial(request):
    ...
    try:
        ...
        profile = EmployeeIndonesiaProfile.objects.filter(employee_id=employee_id).first()
        if profile:
            indonesia["npwp"] = (profile.npwp or "").strip()
            indonesia["nik"] = (profile.nik or "").strip()
            indonesia["bpjs_kesehatan"] = (profile.bpjs_kesehatan or "").strip()
            indonesia["bpjs_ketenagakerjaan"] = (profile.bpjs_ketenagakerjaan or "").strip()
    except Exception:
        pass
    response_data = {
        "department": (
        ...
        "indonesia_npwp": indonesia["npwp"],
        "indonesia_nik": indonesia["nik"],
        "indonesia_bpjs_kesehatan": indonesia["bpjs_kesehatan"],
        "indonesia_bpjs_ketenagakerjaan": indonesia["bpjs_ketenagakerjaan"],
    }
...
def _filefield_to_data_uri(file_field):
    if not file_field:
        return None
    try:
        mime, _enc = guess_type(getattr(file_field, "name", ""))
        if not mime:
...
def _xhtml2pdf_link_callback(uri, _rel):
    ...
    if uri.startswith("data:"):
        return uri
    static_url = getattr(pay_settings, "STATIC_URL", "/static/")
    media_url = getattr(pay_settings, "MEDIA_URL", "/media/")
    if uri.startswith(media_url):
        relative = uri[len(media_url) :].lstrip("/")
        candidate = str(getattr(pay_settings, "MEDIA_ROOT", "")) + "/" + relative
        return candidate
    if uri.startswith(static_url):
        relative = uri[len(static_url) :].lstrip("/")
...
def generate_payslip_pdf(template_path, context, html=False):
    ...
    try:
        ...
        engine = getattr(pay_settings, "HORILLA_PDF_ENGINE", "xhtml2pdf")
        ...
        return HttpResponse("Unsupported PDF engine", status=500)
    except Exception as e:
...
def payslip_pdf(request, id):
    ...
    return redirect(f"{reverse('view-payslip-pdf', kwargs={'payslip_id': id})}?print=1")
